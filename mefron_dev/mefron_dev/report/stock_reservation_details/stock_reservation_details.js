			...
			if (items) {
				if(items[0].to_reserved_qty<=0){
					...
					frappe.call({
						...
						method: "mefron_dev.backend_code.stock_reservation_on_report.create_stock_reservation_entries",
						args: {
							values:items,
							doc:values.so_id
						},
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: () => {
				...
				if (data.sr_entries && data.sr_entries.length > 0) {
					frappe.call({
						// doc: frm.doc,
						method: "mefron_dev.backend_code.stock_reservation_on_report.cancel_stock_reservation_entrie",
						args: {
