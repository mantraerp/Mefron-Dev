...
class PurchaseReceipt(BuyingController):
	...
	def po_required(self):
		# Check if Purchase Order is required globally in Buying Settings
		if frappe.db.get_value("Buying Settings", None, "po_required") == "Yes":
			for d in self.get("items"):
				if not d.purchase_order:
					item_cd=frappe.db.get_value("Item",d.item_code,"custom_allow_purchase_receipt_creation_without_purchase_order")
					if item_cd==0:
