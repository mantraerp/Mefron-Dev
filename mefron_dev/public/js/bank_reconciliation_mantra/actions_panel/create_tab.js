...
erpnext.accounts.bank_reconciliation.CreateTab = class CreateTab {
	...
	create_voucher_bts(allow_edit=false, success_callback) {
		...
		let values = this.create_field_group.get_values();
		let document_type = values.document_type;
		let method = "mefron_dev.mefron_dev.doctype.bank_reconciliation_tool_mantra.bank_reconciliation_tool_mantra";
		let args = {
	...
	reconcile_new_voucher(doctype, docname) {
		...
		var me = this;
		frappe.call({
			method: "mefron_dev.mefron_dev.doctype.bank_reconciliation_tool_mantra.bank_reconciliation_tool_mantra.reconcile_voucher",
			args: {
