                            ...
                            frm.doc.items.forEach((item) => {
                                if (item.name === sales_order_item) {
                                    ...
                                    if (unreserved_qty > 0) {
                                        dialog.fields_dict.items.df.data.forEach((row) => {
                                            ...
                                            if (row.sales_order_item === sales_order_item) {
                                                unreserved_qty -= row.pending_qty;
                                            }
                                        });
                                    }
                                    ...
                                    dialog.fields_dict.items.df.data.push({
                                        ...
                                        pending_qty: Math.max(unreserved_qty, 0),
                                    });
                                    dialog.fields_dict.items.grid.refresh();
                        ...
                        callback: function (response) {
                            if (response.message) {
                                ...
                                response.message.forEach(function (entry) {
                                    var reserved_qty = entry.reserved_qty;
                                    var delivered_qty = entry.delivered_qty;
                                    reserved += reserved_qty - delivered_qty;
                                });
                                var unreserved = item.delivered_qty + reserved;
                                var unreserved_qty = item.qty - unreserved;
                                if (unreserved_qty > 0) {
                                    dialog.fields_dict.items.df.data.push({
                                        __checked: 1,
                                        sales_order_item: item.name,
