...
frappe.ui.form.on('Material Request', {
	...
	before_workflow_action: function(frm) {
		//     // Check for a specific condition before allowing the workflow transition
	...
	refresh: function (frm) {
		...
		if (stock_entry_type) {
			frappe.db.get_value('Stock Entry Type', { 'name': stock_entry_type }, 'add_to_transit', function (data) {
				...
				frappe.call({
					method: "mefron_dev.backend_code.api.warehouse_manager_data_fetch_material_request",
					args: {
		...
		if (cur_frm.doc.transfer_status == "In Transit") {
			frappe.call({
				method: "mefron_dev.backend_code.material_request.get_target_warehouse_deatils",
				args: {
						...
						if (r.message.includes(frappe.session.logged_in_user)) {
							...
							frm.add_custom_button(__('End Transit'), function () {
								...
								frappe.model.open_mapped_doc({
									method: "mefron_dev.backend_code.material_request.make_stock_in_entry",
									frm: frm,
								})
							});
						} else {
